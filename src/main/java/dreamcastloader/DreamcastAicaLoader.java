...
public class DreamcastAicaLoader extends BaseLoader {
	...
	protected void load(Program program, ImporterSettings settings) throws CancelledException, IOException {
		...
		createAicaRegisters(fpa, false, settings.log());
		
		InputStream ramStream = settings.provider().getInputStream(0L);
		createSegment(fpa, ramStream, "RAM", 0, ramSize, true, true, settings.log());
	}
}
